 ...
 */
export default function reportTools(context: Context): Tool<any>[] {
  return [
    generateTestReport,
 ...
 */
function generateSummaryHtmlReport(summaryResults: any): string {
...
<body>
  ...
  <table>
    ...
    ${summaryResults.tests.map((test: any) => `
      <tr>
        <td>${test.testName}</td>
 ...
 */
function generateSummaryMarkdownReport(summaryResults: any): string {
  const passRate = (summaryResults.passed / summaryResults.totalTests) * 100;
...
|-----------|--------|----------|
${summaryResults.tests.map((test: any) => `| ${test.testName} | ${test.passed ? 'PASSED ✅' : 'FAILED ❌'} | ${test.duration}ms |`).join('\n')}
`;
}
