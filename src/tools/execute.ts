 ...
 */
async function executeCypressTest(context: Context, testName: string, fileName: string): Promise<any> {
  try {
    ...
    const { stdout, stderr } = await execPromise(
      `npx cypress run --spec "${testFilePath}" ${recordFlag}`,
      { cwd: tempDir, timeout: 120000 } // 2 minute timeout
    );
    ...
    const screenshots = fs.existsSync(screenshotsDir) 
      ? fs.readdirSync(screenshotsDir, { recursive: true })
          ...
          .map(file => {
            const fullPath = path.join(screenshotsDir, file as string);
            return {
              name: file,
              path: fullPath,
    ...
    if (fs.existsSync(videosDir)) {
      ...
      if (videos.length > 0) {
        ...
        video = {
          ...
          size: fs.statSync(videoPath).size as unknown,
        };
      }
    }
    ...
    if (video) {
      ...
      context.saveArtifact(`${testName}_video`, video as Record<string, unknown>);
    }
