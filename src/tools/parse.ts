...
class NLPProcessor {
  ...
  async parseTestScenario(scenario: string, targetUrl: string): Promise<any> {
  ...
  private simpleParser(scenario: string, targetUrl: string): any {
 ...
 */
export default function parseTools(_context: Context): Tool<any>[] {
 ...
 */
const parseTestScenario = (nlpProcessor: NLPProcessor) => defineTool({
  capability: 'core',
  schema: {
 ...
 */
function parseScenarioToSteps(scenario: string, targetUrl?: string): any[] {
 ...
 */
function extractElement(text: string): string {
